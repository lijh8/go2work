	...
	for i := 0; i != min(len(a), len(b)); i++ {
		...
		if both_side_numeric {
			if a_int_ok && b_int_ok {
				if c := cmp.Compare(a_int, b_int); c != 0 {
					return c, true
				}
			}
			if a_float64_ok && b_float64_ok {
				if c := cmp.Compare(a_float64, b_float64); c != 0 {
					return c, true
				}
			}
			if a_int_ok && b_float64_ok {
				if c := cmp.Compare(float64(a_int), b_float64); c != 0 {
					return c, true
				}
			}
			if a_float64_ok && b_int_ok {
				if c := cmp.Compare(a_float64, float64(b_int)); c != 0 {
					return c, true
				}
			}
		}
		...
		if a_string_ok && b_string_ok {
			if c := cmp.Compare(a_string, b_string); c != 0 {
				return c, true
			}
		}
	}
